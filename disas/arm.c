	  ...
	  for (c = insn->assembler; *c; c++)
	      ...
	      if (*c == '%')
		{
		  switch (*++c)
			...
			c = arm_decode_bitfield (c, given, &new_value, &width);

			switch (*c)
			  {
			  case 'r':
			    func (stream, "%s", arm_regnames[new_value]);
			    break;
			  case 'D':
			    func (stream, "d%ld", new_value);
			    break;
			  case 'Q':
			    if (new_value & 1)
			      func (stream, "<illegal reg q%ld.5>", new_value >> 1);
			    else
			      func (stream, "q%ld", new_value >> 1);
			    break;
			  case 'd':
			    func (stream, "%ld", new_value);
			    break;
                          case 'k':
                            {
                              int from = (given & (1 << 7)) ? 32 : 16;
                              func (stream, "%ld", from - new_value);
                            }
                            break;

			  case 'f':
			    if (new_value > 7)
			      func (stream, "#%s", arm_fp_const[new_value & 7]);
			    else
			      func (stream, "f%ld", new_value);
			    break;

			  case 'w':
			    if (width == 2)
			      func (stream, "%s", iwmmxt_wwnames[new_value]);
			    else
			      func (stream, "%s", iwmmxt_wwssnames[new_value]);
			    break;

			  case 'g':
			    func (stream, "%s", iwmmxt_regnames[new_value]);
			    break;
			  case 'G':
			    func (stream, "%s", iwmmxt_cregnames[new_value]);
			    break;

			  case 'x':
			    func (stream, "0x%lx", new_value);
			    break;
			  ...
			  case '\'':
			    c++;
			    if (new_value == ((1ul << width) - 1))
			      func (stream, "%c", *c);
			    break;
			  case '?':
			    func (stream, "%c", c[(1 << width) - (int)new_value]);
			    c += 1 << width;
			    break;
			  default:
			    abort ();
		      ...
		      case 'Z':
			{
			  ...
			  _new_value = ((given >> 16) & 0xf0) | (given & 0xf);
			  func (stream, "%d", _new_value);
			}
			break;
	  ...
	  for (c = insn->assembler; *c; c++)
	      ...
	      if (*c == '%')
		{
		  switch (*++c)
			...
			if (align)
			  {
                            int _align = (8 * (type + 1)) << size;
                            if (type == 3)
                                _align = (size > 1) ? _align >> 1 : _align;
			    if (type == 2 || (type == 0 && !size))
			      func (stream, ", :<bad align %d>", _align);
			    else
			      func (stream, ", :%d", _align);
			  }
			func (stream, "]");
			if (rm == 0xd)
			  func (stream, "!");
	      ...
	      if (*c == '%')
		{
		  switch (*++c)
			   ...
			   language instruction encodes LSB and MSB.  */
			{
			  ...
			  long _width = msb - lsb + 1;
			  if (_width > 0)
			    func (stream, "#%lu, #%lu", lsb, _width);
			  else
