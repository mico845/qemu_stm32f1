...
static const FlashPartInfo known_devices[] = {
    ...
    /* added by zqzqzq*/
    { INFO("gd25lq32es", 0xc86016,  0xc860,  64 * 1024,  64, ER_4K)},
    { INFO("gd25lq256d", 0xc86019,  0xc860,  64 * 1024,  512, ER_4K)},
    { INFO("gd25q64e",   0xc86017,  0xc860,  64 * 1024,  128, ER_4K)},
    { INFO("gd25q64es",  0xc84017,  0xc840,  64 * 1024,  128, ER_4K)},
    { INFO("gd25ql256d", 0xc86019,  0xc860,  64 * 1024,  512, ER_4K)},
...
static uint32_t m25p80_transfer8(SSIPeripheral *ss, uint32_t tx)
{
    ...
    case STATE_COLLECTING_VAR_LEN_DATA:
        if(tx==FAKER_FLUSH) //zqzqzq
        {
            if(s->cmd_in_progress == PP || s->cmd_in_progress == QPP)
            {
                s->state = STATE_IDLE;
                s->len = s->pos = 0;
                return r;
...
static uint32_t m25p80_transfer_raw(SSIPeripheral *dev, uint32_t val)
{
...
static uint64_t spi_flash_direct_read(void *opaque, hwaddr offset, unsigned int width)
{
    Flash* s = M25P80(opaque);
    uint32_t ret = 0;
    if(width == 1)
    {
        memcpy(&ret, &s->storage[offset], 1);  
    }
    else if(width == 2)
    {
        memcpy(&ret, &s->storage[offset], 2);
    }
    else if(width == 4)
    {
        memcpy(&ret, &s->storage[offset], 4);
    }   
    fprintf(stderr, "Reading from flash at 0x%08x, value is 0x%08x, size is %d\n", offset, ret, width);
    return ret;
}

static void spi_flash_direct_write(void *opaque, hwaddr offset, uint64_t value, unsigned int width)
{
    Flash* s = M25P80(opaque);

    if(s->write_enable && (s->state == STATE_COLLECTING_DATA) && (s->cmd_in_progress == PP || s->cmd_in_progress == QPP))
    {
        memcpy(&s->storage[offset], &value, width);
        s->cur_addr = offset;
        fprintf(stderr, "Writing 0x%08x at 0x%08x, size is %d\n", value, offset, width);
        for(int i = 0; i < width; i++)
        {
            uint8_t tx = (uint8_t)(value >> (i * 8));
            flash_write8(s, s->cur_addr, (uint8_t)tx);
            s->cur_addr = (s->cur_addr + 1) & (s->size - 1);
        }   
    }
...
static const MemoryRegionOps spi_falsh_ops = {
    .read = spi_flash_direct_read,
    .write = spi_flash_direct_write,
...
static void m25p80_realize(SSIPeripheral *ss, Error **errp)
{
    ...
    memory_region_init_io(&s->mem, OBJECT(ss), &spi_falsh_ops, s, "spi-flash-io", s->size );
    if (*errp) {
        return;
    }
    memory_region_add_subregion(get_system_memory(), s->base, &s->mem);
#ifdef RESET_VECTOR
    uint32_t reset_vec[73];
    reset_vec[0] = 0xd5384240;
    reset_vec[1] = 0xd342fc00;
    reset_vec[2] = 0x92400400;
    reset_vec[3] = 0xf1000c1f;
    reset_vec[4] = 0x540003a1;
    reset_vec[5] = 0xd53ecca0;
    reset_vec[6] = 0xb2400c00;
    reset_vec[7] = 0xd51ecca0;
    reset_vec[8] = 0xd5033fdf;
    reset_vec[9] = 0xd53cc9a0;
    reset_vec[10] = 0xb2400c00;
    reset_vec[11] = 0xd51cc9a0;
    reset_vec[12] = 0xd5033fdf;
    reset_vec[13] = 0xd538cca0;
    reset_vec[14] = 0xb2400000;
    reset_vec[15] = 0xd518cca0;
    reset_vec[16] = 0xd5033fdf;
    reset_vec[17] = 0xd2803620;
    reset_vec[18] = 0xd51e1100;
    reset_vec[19] = 0xd2867fe0;
    reset_vec[20] = 0xd51c1140;
    reset_vec[21] = 0xd2810000;
    reset_vec[22] = 0xf2a61a00;
    reset_vec[23] = 0xd5181000;
    reset_vec[24] = 0x910003e0;
    reset_vec[25] = 0xd51c4100;
    reset_vec[26] = 0xd53ec000;
    reset_vec[27] = 0xd518c000;
    reset_vec[28] = 0xd2803a60;
    reset_vec[29] = 0xd51e4000;
    reset_vec[30] = 0x10000500;
    reset_vec[31] = 0xd51e4020;
    reset_vec[32] = 0xd69f03e0;
    reset_vec[33] = 0xd53cc9a0;
    reset_vec[34] = 0xb2400c00;
    reset_vec[35] = 0xd51cc9a0;
    reset_vec[36] = 0xd5033fdf;
    reset_vec[37] = 0xd538cca0;
    reset_vec[38] = 0xb2400000;
    reset_vec[39] = 0xd518cca0;
    reset_vec[40] = 0xd5033fdf;
    reset_vec[41] = 0xd53ce100;
    reset_vec[42] = 0xb2400400;
    reset_vec[43] = 0xd51ce100;
    reset_vec[44] = 0xd51ce07f;
    reset_vec[45] = 0xd5380000;
    reset_vec[46] = 0xd53800a1;
    reset_vec[47] = 0xd51c0000;
    reset_vec[48] = 0xd51c00a1;
    reset_vec[49] = 0xd2867fe0;
    reset_vec[50] = 0xd51c1140;
    reset_vec[51] = 0xd51c117f;
    reset_vec[52] = 0xd2a00600;
    reset_vec[53] = 0xd5181040;
    reset_vec[54] = 0xd2800000;
    reset_vec[55] = 0xb2630000;
    reset_vec[56] = 0xd51c1100;
    reset_vec[57] = 0xd53c1100;
    reset_vec[58] = 0xd2810000;
    reset_vec[59] = 0xf2a61a00;
    reset_vec[60] = 0xd5181000;
    reset_vec[61] = 0x910003e0;
    reset_vec[62] = 0xd51c4100;
    reset_vec[63] = 0xd53cc000;
    reset_vec[64] = 0xd518c000;
    reset_vec[65] = 0xd2803a60;
    reset_vec[66] = 0xd51c4000;
    reset_vec[67] = 0x10000060;
    reset_vec[68] = 0xd51c4020;
    reset_vec[69] = 0xd69f03e0;
    reset_vec[70] = 0xe3000118;
    reset_vec[71] = 0xe3480010;
    reset_vec[72] = 0xe12fff30;
#endif
    if (s->blk) {
    ...
    for (size_t i = 0; i < s->pi->n_sectors; i++)
    {
        memcpy(s->storage + s->pi->sector_size*i, FORMAT_PATTERN, sizeof(FORMAT_PATTERN));
    }
#ifdef RESET_VECTOR
    memcpy(s->storage, reset_vec, sizeof(reset_vec));
#endif
    qdev_init_gpio_in_named(DEVICE(ss), ssi_cs_default, SSI_GPIO_CS, 1);
}
...
static Property m25p80_properties[] = {
    ...
    DEFINE_PROP_UINT64("base", Flash, base, 0x1000),
...
static void m25p80_class_init(ObjectClass *klass, void *data)
{
    ...
    k->transfer_raw = m25p80_transfer_raw;
    k->set_cs = m25p80_cs;
